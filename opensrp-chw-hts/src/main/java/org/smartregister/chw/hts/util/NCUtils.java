...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return HtsLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(HtsLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(HtsLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = HtsLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = HtsLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    HtsLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    HtsLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    HtsLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    HtsLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
