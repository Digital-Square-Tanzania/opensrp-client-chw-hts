...
public class HtsJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    public static final String METADATA = "metadata";

    public static Triple<Boolean, JSONObject, JSONArray> validateParameters(String jsonString) {

        JSONObject jsonForm = toJSONObject(jsonString);
        JSONArray fields = htsFormFields(jsonForm);
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = HtsLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = HtsLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(HtsLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(HtsLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(HtsLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
        ...
        for (Map.Entry<String, String> map : jsonStrings.entrySet()) {
            ...
            while (local_fields.length() > x) {
                try {
                    ...
                    obj.put(Hts_VISIT_GROUP, map.getKey());
                    fields_obj.add(obj);
                } catch (JSONException e) {
                    Timber.e(e);
                }
